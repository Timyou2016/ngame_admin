<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="父级部门" align="center" width="200">
        <template slot-scope="scope">
          {{ scope.row.parent_id | showDepartment }}
          <el-link v-if="scope.row.parent_id > 0" :underline="false" @click="showParent(scope.row.parent_id)">查看<i class="el-icon-view el-icon--right"></i> </el-link>
        </template>
      </el-table-column>         
      <el-table-column label="部门名称" align="center" width="200">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
...
export default {
  name: 'DepartmentList',
  components: { Pagination , SelectDepartment},
  directives: { waves },
  filters: {
    statusFilter(status) {
      const statusMap = {
